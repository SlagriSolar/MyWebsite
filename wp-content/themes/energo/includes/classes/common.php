<?php namespace ENERGO\Includes\Classes;
/**
 * Common functions
 */
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => energo_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => energo_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => energo_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => energo_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => energo_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => energo_meta( 'banner_elementor_template', $page_id ),
				'banner'            => energo_set( energo_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => energo_meta( 'banner_banner_title', $page_id ),
				'banner_text'       => energo_meta( 'banner_banner_text', $page_id ),
				'enable_banner'     => energo_meta( 'banner_page_banner', $page_id ),
			];
			...
			$banner_text   = $template . '_banner_text';
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : energo_the_title( $template ),
				'banner_text'   => $options->get( $banner_text ),
				'banner'        => energo_set( $bg, 'url' ),
